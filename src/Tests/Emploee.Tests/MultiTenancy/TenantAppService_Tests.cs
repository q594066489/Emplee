...
using Emploee.MultiTenancy.Dto;
using Emploee.Notifications;
using Shouldly;
using Xunit;

namespace Emploee.Tests.MultiTenancy
{
        ...
        public async Task Create_Update_And_Delete_Tenant_Test()
        {
            ...
            await _tenantAppService.CreateTenant(
                new CreateTenantInput
                {
                    ...
                    IsActive = true,
                    ConnectionString = "Server=localhost; Database=EmploeeTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;"
                });
