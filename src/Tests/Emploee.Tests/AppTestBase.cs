...
using Emploee.Migrations.Seed.Host;
using Emploee.Migrations.Seed.Tenants;
using Emploee.MultiTenancy;
using Emploee.Tests.TestDatas;
...
namespace Emploee.Tests
{
    ...
    /// Provides methods to easily work with <see cref="EmploeeDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<EmploeeTestModule>
    {
        ...
        protected void UsingDbContext(Action<EmploeeDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EmploeeDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EmploeeDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EmploeeDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EmploeeDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EmploeeDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EmploeeDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EmploeeDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EmploeeDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EmploeeDbContext, Task<T>> func)
        {
            T result;
