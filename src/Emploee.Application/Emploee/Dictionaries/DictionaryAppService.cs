...
DictionaryManage dictionaryManage
        ...
        public async Task<PagedResultDto<DictionaryListDto>> GetPagedDictionarysAsync(GetDictionaryInput input)
        {
            ...
            var query = _dictionaryRepositoryAsNoTrack
                ...
                .Where(t=>t.ParentCode==input.ParentCode)
                .WhereIf(!string.IsNullOrEmpty(input.FilterText),t=>t.Name==input.FilterText.Trim());
            //TODO:根据传入的参数添加过滤条件
        ...
        public async Task<ListResultDto<DictionaryListDto>> GetDictionaryLists(string ParentCode)
        {

            var query = _dictionaryRepositoryAsNoTrack;
            //TODO:根据传入的参数添加过滤条件
            query = query.Where(q => q.ParentCode.ToString() == (ParentCode ?? ""));

            var newquery = await query.ToListAsync();

            return new ListResultDto<DictionaryListDto>(newquery.MapTo<List<DictionaryListDto>>());
        }
        /// <summary>
        /// 获取父类信息
        /// </summary>
        ...
        /// <returns></returns>
        public List<DictionaryListDto> GetParentList(string parentCode)
        {
            List<DictionaryListDto> list = new List<DictionaryListDto>();
            if (string.IsNullOrEmpty(parentCode)) return list;
            var query = _dictionaryRepositoryAsNoTrack;
            var category = query.FirstOrDefault(c => c.Code == parentCode);
            if (category != null && !string.IsNullOrEmpty(category.ParentCode))
            {
                var parent = query.FirstOrDefault(c => c.Code == category.ParentCode);
                list.Add(parent.MapTo<DictionaryListDto>());
                if (parent != null && !string.IsNullOrEmpty(parent.ParentCode))
                {
                    GetParents(parent.ParentCode, list);
                }
                list.Add(category.MapTo<DictionaryListDto>());
        ...
        private void GetParents(string parentCode, List<DictionaryListDto> list)
        {
            var query = _dictionaryRepositoryAsNoTrack;
            var category = query.FirstOrDefault(c => c.Code == parentCode);
            list.Add(category.MapTo<DictionaryListDto>());
            if (category != null && !string.IsNullOrEmpty(category.ParentCode))
            {
                GetParents(category.ParentCode, list);
            }
        }
        /// <summary>
        /// 通过Id获取数据字典信息进行编辑或修改 
